...
**CubePi v<PackageVersion /> is the latest release.** CubePi 0.8 adds
conversation forking (`Agent.fork()` / `Agent.fork_once()`), run-based
checkpointing (schema v3→v4 across all backends), the `@tool` decorator for
ergonomic tool definition, and `Agent.prompt()` now returns the `run_id`.
