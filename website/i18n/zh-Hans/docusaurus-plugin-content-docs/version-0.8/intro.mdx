...
**CubePi v<PackageVersion /> 是最新发布版本。** CubePi 0.8 新增会话分叉
（`Agent.fork()` / `Agent.fork_once()`）、基于 run 的 checkpointing
（全后端 schema v3→v4）、`@tool` 装饰器（便捷工具定义），以及
`Agent.prompt()` 现在会返回 `run_id`。
