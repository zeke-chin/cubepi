...
你正在浏览的是 **CubePi 0.11** 发布文档。这是安装 CubePi 并基于已发布
0.11 API 进行开发的最新稳定文档集。

CubePi 0.11 将 deferred tool group 的默认策略切换为 `dispatch`
（通过 `load_tools` 结果和 `deferred_tool_call` 分发器实现
prompt-cache 稳定的扩展），新增 `resolve_tool_call` 中间件 hook
（在验证和执行之前重写 tool call），以及 `AgentTool.expose_to_model`
（隐藏但可解析的工具）。完整变更请查看 [Changelog](/changelog)。
