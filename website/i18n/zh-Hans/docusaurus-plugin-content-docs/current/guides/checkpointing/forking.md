...
- **Postgres** — 参见 [Postgres → Schema v3→v4](./postgres#schema-v3--v4-migration)。
- **MySQL** — 参见 [MySQL → Schema v3→v4](./mysql#schema-v3--v4-migration)。
...
- [Postgres 检查点](./postgres) — 支持 v4 schema 的生产后端。
- [MySQL 检查点](./mysql) — 支持 v4 schema 的 MySQL 后端。
- [SQLite 检查点](./sqlite) — 支持自动迁移的单进程后端。
- [自定义后端](./custom) — Protocol 详情，包括分叉支持所需实现的
